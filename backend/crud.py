...
async def create_request(
    db: AsyncSession,
    type: models.ApprovalActionType,
    requester_id: int,
    category_id: int,
    data: dict,
    ...
    db.add(db_request)
    # Flush to get ID, but not commit yet to ensure atomicity with item updates if needed
    await db.flush()

    # Link items to request
    for item in items:
        item.request_id = db_request.id
...
async def get_user_requests(db: AsyncSession, user_id: int):
    query = select(models.Request).where(models.Request.requester_id == user_id).options(
        selectinload(models.Request.items),
        selectinload(models.Request.category),
        selectinload(models.Request.requester) # Self reference, but good for consistency
    ).order_by(models.Request.created_at.desc())
    result = await db.execute(query)
    return result.scalars().all()

async def get_pending_requests(db: AsyncSession):
    query = select(models.Request).where(
...
async def finalize_request(db: AsyncSession, request: models.Request):
    """Applies the final action to all items in the request."""

    # Assuming request.items is loaded
    for item in request.items:
        if request.type == models.ApprovalActionType.WRITE_OFF:
            item.status = models.ItemStatus.WRITTEN_OFF
            # Log
            log = models.Log(item_id=item.id, user_id=request.requester_id, action="Baixa Aprovada e Concluída via Solicitação #" + str(request.id))
            db.add(log)

        elif request.type == models.ApprovalActionType.TRANSFER:
            item.status = models.ItemStatus.IN_TRANSIT
            # Log
            log = models.Log(item_id=item.id, user_id=request.requester_id, action="Transferência Aprovada e Em Trânsito via Solicitação #" + str(request.id))
            db.add(log)

    await db.commit()

async def reject_request_items(db: AsyncSession, request: models.Request):
    """Reverts items to their original state (or just Active)."""

    for item in request.items:
        # Revert status. Logic depends on previous status.
        # Simplification: If WRITE_OFF or TRANSFER was pending, revert to APPROVED (if it was active) or IN_STOCK?
        # Ideally we should revert to 'APPROVED' as safe default or check history.
        # But 'APPROVED' is the standard 'Active' state.
        item.status = models.ItemStatus.APPROVED
        item.transfer_target_branch_id = None # Clear transfer intent if any
        item.write_off_reason = None
        item.request_id = None # Unlink from request? Or keep for history?
        # Keeping request_id is good for audit, but if we want to allow re-requesting, maybe we should keep it but status is REJECTED on Request.
        # The Item status is what locks it.

        log = models.Log(item_id=item.id, user_id=request.requester_id, action="Solicitação #" + str(request.id) + " Rejeitada. Item liberado.")
        db.add(log)
